...
<head>
  ...
  <title>{{ title }} | FFOS</title>
  <link href="https://fonts.googleapis.com/css2?family=IBM+Plex+Mono:wght@300;400;500;600&family=IBM+Plex+Sans:wght@300;400;500;600&display=swap" rel="stylesheet">
  <style>
    :root{--bg:#080e0a;--surface:#0d1610;--surface2:#111e14;--border:#1e3322;
          --green:#00e87a;--green2:#00c060;--amber:#f5a623;--red:#ff4d4d;
          --text:#c8dcc8;--muted:#5a7a5a;--mono:'IBM Plex Mono',monospace;--sans:'IBM Plex Sans',sans-serif;}
    *,*::before,*::after{box-sizing:border-box;margin:0;padding:0;}
    body{background:var(--bg);color:var(--text);font-family:var(--sans);font-size:14px;line-height:1.6;min-height:100vh;}
    body::before{content:'';position:fixed;inset:0;pointer-events:none;z-index:9999;
      background:repeating-linear-gradient(0deg,transparent,transparent 2px,rgba(0,0,0,.03) 2px,rgba(0,0,0,.03) 4px);}
    nav{background:var(--surface);border-bottom:1px solid var(--border);padding:0 1.5rem;height:52px;
        display:flex;align-items:center;justify-content:space-between;position:sticky;top:0;z-index:100;}
    .nav-brand{font-family:var(--mono);font-size:.9rem;font-weight:600;color:var(--green);
               text-decoration:none;letter-spacing:.12em;text-transform:uppercase;}
    .nav-brand::before{content:'> ';color:var(--muted);}
    nav ul{list-style:none;display:flex;gap:.25rem;align-items:center;flex-wrap:wrap;}
    nav a{color:var(--muted);text-decoration:none;font-family:var(--mono);font-size:.78rem;
          padding:.35rem .75rem;border:1px solid transparent;border-radius:3px;transition:all .15s;}
    nav a:hover{color:var(--green);border-color:var(--border);}
    nav a.active{color:var(--green);border-color:var(--green);}
    .btn-nav-cta{background:var(--green)!important;color:var(--bg)!important;font-weight:600!important;border-color:var(--green)!important;}
    ...
    h1,h2,h3,h4{font-family:var(--mono);font-weight:600;color:#d8edd8;}
    h1{font-size:1.6rem;margin-bottom:.5rem;}h2{font-size:1.2rem;margin-bottom:.5rem;}
    .mono{font-family:var(--mono);}.muted{color:var(--muted);}.green{color:var(--green);}
    .amber{color:var(--amber);}.red{color:var(--red);}
    .bar-wrap{background:var(--border);border-radius:2px;height:6px;overflow:hidden;}
    .bar{height:6px;border-radius:2px;transition:width .4s;}
    .bar-green{background:var(--green);}.bar-amber{background:var(--amber);}.bar-red{background:var(--red);}
    .stat-box{background:var(--surface2);border:1px solid var(--border);border-radius:4px;padding:1rem;}
    .stat-box .val{font-family:var(--mono);font-size:1.4rem;font-weight:600;color:#d8edd8;margin-bottom:.15rem;}
    .stat-box .lbl{font-size:.75rem;color:var(--muted);letter-spacing:.05em;text-transform:uppercase;}
    .alert-item{padding:.65rem 1rem;border-left:3px solid var(--border);margin-bottom:.5rem;
                font-family:var(--mono);font-size:.8rem;background:var(--surface2);border-radius:0 3px 3px 0;}
    .alert-item.warn{border-color:var(--amber);color:var(--amber);}
    .alert-item.ok{border-color:var(--green);color:var(--green);}
    label{display:block;font-size:.75rem;letter-spacing:.06em;text-transform:uppercase;color:var(--muted);margin-bottom:.3rem;}
    input[type=text],input[type=email],input[type=password],input[type=number],input[type=file],select,textarea{
      width:100%;background:var(--bg);border:1px solid var(--border);color:var(--text);
      font-family:var(--mono);font-size:.88rem;padding:.55rem .85rem;border-radius:3px;outline:none;transition:border-color .15s;}
    input:focus,select:focus,textarea:focus{border-color:var(--green);}
    .form-group{margin-bottom:1rem;}
    .btn{display:inline-block;font-family:var(--mono);font-size:.8rem;font-weight:500;letter-spacing:.06em;
         padding:.55rem 1.25rem;border-radius:3px;border:1px solid var(--border);cursor:pointer;
         text-decoration:none;color:var(--text);background:transparent;transition:all .15s;white-space:nowrap;}
    .btn:hover{border-color:var(--green);color:var(--green);}
    .btn-primary{background:var(--green);color:var(--bg);border-color:var(--green);font-weight:600;}
    .btn-primary:hover{background:var(--green2);border-color:var(--green2);color:var(--bg);}
    .btn-danger{border-color:var(--red);color:var(--red);}
    .btn-amber{border-color:var(--amber);color:var(--amber);}
    .btn-sm{padding:.35rem .8rem;font-size:.74rem;}.btn-block{display:block;width:100%;text-align:center;}
    table{width:100%;border-collapse:collapse;font-size:.82rem;}
    th{font-family:var(--mono);font-size:.7rem;letter-spacing:.08em;text-transform:uppercase;
       color:var(--muted);border-bottom:1px solid var(--border);padding:.5rem .75rem;text-align:left;}
    td{padding:.55rem .75rem;border-bottom:1px solid rgba(30,51,34,.5);font-family:var(--mono);}
    ...
    .flash-item{padding:.65rem 1rem;border-left:3px solid var(--muted);margin-bottom:.4rem;
                font-family:var(--mono);font-size:.8rem;background:var(--surface2);}
    .flash-item.success{border-color:var(--green);color:var(--green);}
    .flash-item.warning{border-color:var(--amber);color:var(--amber);}
    hr{border:none;border-top:1px solid var(--border);margin:1.25rem 0;}
    .badge{display:inline-block;font-family:var(--mono);font-size:.68rem;padding:.15rem .5rem;border-radius:2px;letter-spacing:.05em;}
    ...
    .badge-pink{background:rgba(232,93,117,.12);color:#e85d75;border:1px solid rgba(232,93,117,.25);}
    .badge-muted{background:rgba(90,122,90,.12);color:var(--muted);border:1px solid var(--border);}
    .section-label{font-family:var(--mono);font-size:.68rem;letter-spacing:.12em;text-transform:uppercase;color:var(--muted);margin-bottom:.75rem;}
    .section-label::before{content:'// ';}
    footer{border-top:1px solid var(--border);padding:1rem 1.5rem;font-family:var(--mono);font-size:.72rem;color:var(--muted);text-align:center;}
    @keyframes fadeIn{from{opacity:0;transform:translateY(6px)}to{opacity:1;transform:none}}
    .fade-in{animation:fadeIn .25s ease both;}
    .cursor::after{content:'▋';animation:blink 1s step-end infinite;color:var(--green);}
    @keyframes blink{50%{opacity:0}}
    input[type=radio]{accent-color:var(--green);width:auto;}
  </style>
</head>
<body>
<nav>
  <a class="nav-brand" href="{{ url_for('home') }}">FFOS</a>
  <ul>
    {% if u %}
      <li><a href="{{ url_for('dashboard') }}">Dashboard</a></li>
      <li><a href="{{ url_for('scoreboard') }}">Score</a></li>
      <li><a href="{{ url_for('edu_learn') }}" class="{{ 'active' if active_page=='learn' else '' }}">Learn</a></li>
      <li><a href="{{ url_for('budget') }}">Budget</a></li>
      <li><a href="{{ url_for('transactions') }}">Cashflow</a></li>
      <li><a href="{{ url_for('debt') }}">Debt</a></li>
      <li><a href="{{ url_for('retirement') }}">Retire</a></li>
...
<footer>LEGACYLIFT LLC &nbsp;|&nbsp; EDUCATION ONLY — NOT FINANCIAL ADVICE &nbsp;|&nbsp; &copy; {{ year }} &nbsp;|&nbsp; <a href="/privacy" style="color:var(--muted)">Privacy</a> &nbsp;·&nbsp; <a href="/terms" style="color:var(--muted)">Terms</a> &nbsp;·&nbsp; <a href="/about" style="color:var(--muted)">Our Story</a></footer>
</body>
</html>"""
...
.ll-page{background:#09090b;min-height:100vh;margin:-1rem;padding:0;}
.ll-hero{position:relative;overflow:hidden;padding:0;background:linear-gradient(135deg,#09090b 0%,#0f0f14 100%);}
.ll-glow{position:absolute;inset:0;pointer-events:none;background:radial-gradient(ellipse 80% 60% at 30% 20%,rgba(124,106,245,.09),transparent 60%),radial-gradient(ellipse 60% 40% at 80% 80%,rgba(200,168,75,.07),transparent 60%);}
.ll-nav{display:flex;align-items:center;justify-content:space-between;padding:1.4rem 3rem;position:relative;z-index:10;background:rgba(9,9,11,.8);backdrop-filter:blur(10px);border-bottom:1px solid rgba(255,255,255,.06);}
.ll-brand-wrap{display:flex;align-items:center;gap:.6rem;}
.ll-brand-ic{width:32px;height:32px;border-radius:8px;background:linear-gradient(135deg,#7c6af5,#c8a84b);display:flex;align-items:center;justify-content:center;font-weight:700;font-size:.8rem;color:#fff;font-family:monospace;box-shadow:0 0 14px rgba(124,106,245,.3);}
.ll-brand-txt{font-family:'Cormorant Garamond',serif;font-size:1.25rem;font-weight:600;background:linear-gradient(135deg,#a89cf8,#e2c97e);-webkit-background-clip:text;-webkit-text-fill-color:transparent;background-clip:text;}
.ll-nav-r{display:flex;align-items:center;gap:.5rem;flex-wrap:wrap;}
.ll-nbtn{background:rgba(255,255,255,.06);border:1px solid rgba(255,255,255,.1);color:#c8d0c8;padding:.42rem 1rem;border-radius:8px;text-decoration:none;font-size:.8rem;font-family:'DM Sans',sans-serif;transition:all .18s;}
.ll-nbtn:hover{background:rgba(255,255,255,.1);color:#f0eeea;}
.ll-nbtn-cta{background:linear-gradient(135deg,#7c6af5,#5948d4);color:#fff;padding:.45rem 1.25rem;border-radius:8px;text-decoration:none;font-size:.82rem;font-weight:600;font-family:'DM Sans',sans-serif;box-shadow:0 4px 14px rgba(124,106,245,.3);transition:all .2s;}
.ll-nbtn-cta:hover{box-shadow:0 6px 20px rgba(124,106,245,.5);transform:translateY(-1px);}
.ll-hero-body{text-align:center;padding:5rem 2rem 4rem;position:relative;z-index:1;}
.ll-pill{display:inline-block;font-size:.65rem;letter-spacing:.16em;text-transform:uppercase;color:#a89cf8;border:1px solid rgba(124,106,245,.3);padding:.3rem 1rem;border-radius:20px;margin-bottom:1.4rem;background:rgba(124,106,245,.08);font-family:'DM Sans',sans-serif;}
.ll-h1{font-family:'Cormorant Garamond',serif;font-size:clamp(2.6rem,6vw,5rem);color:#f0eeea;line-height:1.06;margin:0 auto 1.1rem;max-width:820px;}
.ll-gold{background:linear-gradient(135deg,#e2c97e,#c8a84b);-webkit-background-clip:text;-webkit-text-fill-color:transparent;background-clip:text;}
.ll-lift{background:linear-gradient(135deg,#a89cf8,#7c6af5);-webkit-background-clip:text;-webkit-text-fill-color:transparent;background-clip:text;}
.ll-sub{font-size:1.02rem;color:#6b6b78;max-width:560px;margin:0 auto 2rem;line-height:1.8;font-family:'DM Sans',sans-serif;}
.ll-btns{display:flex;gap:.85rem;justify-content:center;flex-wrap:wrap;margin-bottom:2rem;}
.ll-btn-p{background:linear-gradient(135deg,#7c6af5,#5948d4);color:#fff;padding:.88rem 2.3rem;border-radius:10px;text-decoration:none;font-weight:700;font-size:.95rem;transition:all .2s;box-shadow:0 4px 18px rgba(124,106,245,.35);font-family:'DM Sans',sans-serif;}
.ll-btn-p:hover{box-shadow:0 8px 28px rgba(124,106,245,.5);transform:translateY(-2px);}
.ll-btn-s{background:rgba(255,255,255,.06);color:#f0eeea;padding:.88rem 2rem;border-radius:10px;text-decoration:none;font-weight:600;font-size:.95rem;border:1px solid rgba(255,255,255,.12);transition:all .18s;font-family:'DM Sans',sans-serif;}
.ll-btn-s:hover{background:rgba(255,255,255,.09);border-color:rgba(200,168,75,.35);}
.ll-trust{display:flex;gap:1.5rem;justify-content:center;flex-wrap:wrap;font-size:.74rem;color:#3a3a42;font-family:'DM Sans',sans-serif;}
.ll-trust span::before{content:'✦ ';color:#c8a84b;font-size:.58rem;}
.ll-strip{background:#0d0d10;border-top:1px solid rgba(255,255,255,.05);border-bottom:1px solid rgba(255,255,255,.05);padding:.8rem 2rem;display:flex;justify-content:center;gap:2.5rem;flex-wrap:wrap;}
.ll-si{font-size:.72rem;color:#6b6b78;display:flex;gap:.4rem;align-items:center;font-family:'DM Sans',sans-serif;}
.ll-si strong{color:#e2c97e;font-weight:600;}
.ll-sect{padding:5rem 2rem;background:#09090b;}
.ll-sect-inner{max-width:1100px;margin:0 auto;}
.ll-stag{font-size:.63rem;letter-spacing:.14em;text-transform:uppercase;color:#a89cf8;background:rgba(124,106,245,.1);padding:.22rem .65rem;border-radius:4px;border:1px solid rgba(124,106,245,.2);display:inline-block;margin-bottom:.65rem;font-family:'DM Sans',sans-serif;}
.ll-stitle{font-family:'Cormorant Garamond',serif;font-size:2.2rem;font-weight:600;color:#f0eeea;margin-bottom:.4rem;}
.ll-ssub{font-size:.88rem;color:#6b6b78;margin-bottom:2.5rem;font-family:'DM Sans',sans-serif;}
.ll-grid{display:grid;grid-template-columns:repeat(auto-fill,minmax(275px,1fr));gap:1rem;}
.ll-card{background:#111114;border:1px solid rgba(255,255,255,.07);border-radius:14px;padding:1.5rem;text-decoration:none;display:block;transition:all .22s;}
.ll-card:hover{border-color:rgba(124,106,245,.35);transform:translateY(-3px);box-shadow:0 12px 32px rgba(0,0,0,.4);}
.ll-card-ic{font-size:1.8rem;margin-bottom:.85rem;}
.ll-card-t{font-family:'Cormorant Garamond',serif;font-size:1.1rem;font-weight:600;color:#f0eeea;margin-bottom:.35rem;}
.ll-card-d{font-size:.8rem;color:#6b6b78;line-height:1.65;font-family:'DM Sans',sans-serif;}
.ll-tests{padding:4rem 2rem;background:#0d0d10;}
.ll-tgrid{display:grid;grid-template-columns:repeat(auto-fill,minmax(300px,1fr));gap:1rem;max-width:1100px;margin:0 auto;}
.ll-tc{background:#111114;border:1px solid rgba(255,255,255,.07);border-radius:12px;padding:1.4rem;}
.ll-ts{color:#c8a84b;font-size:.88rem;letter-spacing:.08em;margin-bottom:.7rem;}
.ll-tb{font-size:.84rem;color:#6b6b78;line-height:1.7;font-style:italic;margin-bottom:.65rem;font-family:'DM Sans',sans-serif;}
.ll-ta{font-size:.74rem;font-weight:600;color:#a89cf8;font-family:'DM Sans',sans-serif;}
.ll-cta{padding:5rem 2rem;text-align:center;background:linear-gradient(135deg,rgba(124,106,245,.1),rgba(200,168,75,.06));border-top:1px solid rgba(255,255,255,.06);}
.ll-cta-t{font-family:'Cormorant Garamond',serif;font-size:2.6rem;font-weight:600;color:#f0eeea;margin-bottom:.65rem;}
.ll-cta-s{font-size:.9rem;color:#6b6b78;margin-bottom:1.75rem;font-family:'DM Sans',sans-serif;}
</style>
...
<div class="ll-hero">
  ...
  <div class="ll-hero-body">
    <div class="ll-pill">Generational Wealth Platform</div>
    <h1 class="ll-h1"><span class="ll-gold">Lift your family.</span><br/><span class="ll-lift">Build your legacy.</span></h1>
    <p class="ll-sub">The complete financial intelligence platform for families &mdash; Freedom Score, debt elimination, retirement projections, and a money curriculum for adults <em>and</em> children.</p>
    <div class="ll-btns">
      <a href="/signup" class="ll-btn-p">Begin Your Legacy &mdash; Free &rarr;</a>
      <a href="/pricing" class="ll-btn-s">View Pricing</a>
    </div>
    <div class="ll-trust">
      <span>Free to start</span><span>12 curated lessons</span><span>Adults + Children</span><span>Cancel anytime</span>
    </div>
  </div>
</div>
...
<div class="ll-strip">
  ...
  <div class="ll-si"><strong>Education</strong> 12 lessons + quizzes</div>
</div>

<div class="ll-sect">
  <div class="ll-sect-inner">
    <div style="text-align:center;margin-bottom:3rem;">
      <div class="ll-stag">Platform Features</div>
      <div class="ll-stitle">Everything your family needs.</div>
      <div class="ll-ssub">Real tools. Real results. No fluff.</div>
    </div>
    <div class="ll-grid">
      <a href="/signup" class="ll-card"><div class="ll-card-ic">&#x1F4CA;</div><div class="ll-card-t">Freedom Score (0&ndash;100)</div><div class="ll-card-d">Real-time composite health score across cashflow, emergency fund, debt burden, investing rate, and spending leaks.</div></a>
      <a href="/signup" class="ll-card"><div class="ll-card-ic">&#x1F4C1;</div><div class="ll-card-t">Cashflow Analyzer</div><div class="ll-card-d">Upload your bank CSV. Every transaction auto-categorized. Your biggest spending leaks surfaced in 60 seconds. No bank login required.</div></a>
      <a href="/signup" class="ll-card"><div class="ll-card-ic">&#x1F4A3;</div><div class="ll-card-t">Debt Destruction Engine</div><div class="ll-card-d">DOLP, Avalanche, or Snowball. A precise month-by-month schedule showing your exact debt-free date.</div></a>
      <a href="/signup" class="ll-card"><div class="ll-card-ic">&#x1F4C8;</div><div class="ll-card-t">Retirement Planner</div><div class="ll-card-d">Future value projections with decade milestones and 4%-rule income estimates. See exactly what your investing becomes in 20 years.</div></a>
      <a href="/signup" class="ll-card"><div class="ll-card-ic">&#x1F4DA;</div><div class="ll-card-t">Education Curriculum</div><div class="ll-card-d">12 curated lessons for adults and children ages 5&ndash;14 &mdash; with quizzes, progress tracking, and generational wealth habits.</div></a>
      <a href="/org" class="ll-card"><div class="ll-card-ic">&#x1F3DB;</div><div class="ll-card-t">B2B Licensing</div><div class="ll-card-d">License seats to employers, credit unions, and schools. All organization members receive Premium access automatically.</div></a>
    </div>
  </div>
</div>
...
<div class="ll-tests">
  <div style="text-align:center;margin-bottom:2.5rem;">
    <div class="ll-stag">Real Families</div>
    <div class="ll-stitle">Built for people like you.</div>
  </div>
  <div class="ll-tgrid">
    ...
    <div class="ll-tc"><div class="ll-ts">&#x2736; &#x2736; &#x2736; &#x2736; &#x2736;</div><div class="ll-tb">"We did the children&rsquo;s curriculum together. My daughter now asks &lsquo;need or want?&rsquo; before every purchase &mdash; completely unprompted. That&rsquo;s a generational win."</div><div class="ll-ta">@priya_k &mdash; My daughter manages money at 10</div></div>
    <div class="ll-tc"><div class="ll-ts">&#x2736; &#x2736; &#x2736; &#x2736; &#x2736;</div><div class="ll-tb">"I&rsquo;m 44. Raising my monthly investing from $200 to $550 produces $280K more at 65. The retirement planner showed me what I was leaving behind."</div><div class="ll-ta">@james_r &mdash; Retirement strategy transformed</div></div>
  </div>
</div>
...
<div class="ll-cta">
  ...
  <a href="/signup" class="ll-btn-p" style="display:inline-block;padding:.9rem 2.5rem;font-size:1rem;">Create Your Free Account &rarr;</a>
</div>
</div>"""
